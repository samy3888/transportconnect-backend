...
// Nouvelle colonne chauffeur_id sur demandes
'ALTER TABLE demandes ADD COLUMN IF NOT EXISTS chauffeur_id INTEGER',
];
...
// ============================================================
// RESET SOCIETE1 ADMIN
// ============================================================
bcrypt.hash('Test1234', 10).then(hash => {
pool.query(
`INSERT INTO societes (nom, email, mot_de_passe, actif, is_admin, ville, type_transport)
VALUES ('Société Test', 'societe1@test.fr', $1, true, true, 'Gillonnay', 'Médical (VSL/Ambulance)')
ON CONFLICT (email) DO UPDATE SET actif = true, is_admin = true`,
[hash]
).then(() => console.log('societe1 OK')).catch(e => console.log(e.message));
});
...
email VARCHAR(255) UNIQUE, mot_de_passe VARCHAR(255),
telephone VARCHAR(50), adresse VARCHAR(255),
ville VARCHAR(255), code_postal VARCHAR(20),
date_naissance VARCHAR(50), contact_urgence VARCHAR(255),
tel_urgence VARCHAR(50), is_mineur BOOLEAN DEFAULT false,
...
[chauffeur_id || null, patient_id, adresse_depart, adresse_arrivee, date, patient_token, req.user.societe_id || req.user.id, type_transport, mobilite, accompagnant, urgent, commentaire, patient_telephone]
);
...
[chauffeur_id || null, patient_id, adresse_depart, adresse_arrivee, date, urgent, commentaire, req.params.id]
);
...
try {
await pool.query('UPDATE trajets SET status=$1 WHERE id=$2', [req.body.status, req.params.id]);
res.json({ success: true });
} catch (e) { res.status(500).json({ error: e.message }); }
});
...
try {
await pool.query('UPDATE trajets SET status=$1, commentaire=$2 WHERE id=$3', ['refuse', req.body.motif, req.params.id]);
res.json({ success: true });
} catch (e) { res.status(500).json({ error: e.message }); }
});
...
// ---- ROUTE PRINCIPALE : RÉPONDRE À UNE DEMANDE ----
// Modifiée pour accepter chauffeur_id optionnel et créer le trajet avec chauffeur assigné
app.put('/api/demandes/:id/repondre', authenticateToken, async (req, res) => {
const { statut, chauffeur_id } = req.body;
try {
...
// Créer le trajet avec chauffeur_id si fourni
await pool.query(
`INSERT INTO trajets (patient_id, adresse_depart, adresse_arrivee, date, status, patient_token, societe_id, chauffeur_id, type_transport, mobilite, accompagnant, urgent, commentaire)
VALUES ($1,$2,$3,$4,'en_attente',$5,$6,$7,$8,$9,$10,$11,$12)`,
[d.patient_nom, d.adresse_depart, d.adresse_arrivee, d.date, patient_token, d.societe_id, chauffeur_id || null, d.type_transport, d.mobilite, d.accompagnant, d.urgent, d.commentaire]
);

await pool.query('UPDATE demandes SET patient_token=$1, chauffeur_id=$2 WHERE id=$3', [patient_token, chauffeur_id || null, req.params.id]);

return res.json({ success: true, patient_token });
}
...
try {
await pool.query(
'UPDATE demandes SET statut=$1 WHERE id=$2 AND patient_id=$3',
['annule', req.params.id, req.user.id]
);
res.json({ success: true });
} catch (e) { res.status(500).json({ error: e.message }); }
});
...
try {
const result = await pool.query(
'SELECT id, nom, prenom, email, telephone, ville, created_at FROM patients_comptes ORDER BY created_at DESC'
);
res.json(result.rows);
} catch (e) { res.status(500).json({ error: e.message }); }
});
...
try {
await pool.query('UPDATE societes SET actif=$1 WHERE id=$2', [req.body.actif, req.params.id]);
res.json({ success: true });
} catch (e) { res.status(500).json({ error: e.message }); }
});
...
const server = app.listen(PORT, () => { console.log('TransportConnect démarré port ' + PORT); });
...
wss.clients.forEach(client => { if (client !== ws && client.readyState === 1) client.send(JSON.stringify(data)); });
} catch (e) {}
});
});
